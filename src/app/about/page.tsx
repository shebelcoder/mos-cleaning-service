...
const values = [
  { icon: Shield, title: "Licensed & Insured", desc: "All our cleaners are fully insured, background-checked, and trained professionals." },
  { icon: Star, title: "Quality Guaranteed", desc: "We're not satisfied until you are. We'll return and re-clean at no extra charge." },
  { icon: Users, title: "Experienced Team", desc: "Our cleaners bring years of experience in residential and commercial cleaning." },
  { icon: CheckCircle, title: "Eco-Friendly", desc: "We use safe, environmentally friendly products that are great for families and pets." },
];
    ...
    <div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl lg:text-4xl font-extrabold mb-6" style={{ color: "#003d54" }}>Our Story</h2>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={stat.label} className="rounded-2xl p-6 text-center border" style={{ backgroundColor: "#f1f4f4", borderColor: "#e5e7eb" }}>
                  <div className="text-4xl font-extrabold" style={{ color: "#00adee" }}>{stat.number}</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((v) => (
              <div key={v.title} className="bg-white rounded-2xl p-6 shadow-sm text-center hover:shadow-md transition-shadow">
                <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: "#00adee1a" }}>
                  <v.icon size={26} style={{ color: "#00adee" }} />
                </div>
                <h3 className="font-bold mb-2 text-sm" style={{ color: "#003d54" }}>{v.title}</h3>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-3xl p-10 text-white text-center" style={{ backgroundColor: "#003d54" }}>
            <div className="inline-block mb-4 px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider" style={{ backgroundColor: "#00adee22", color: "#00adee" }}>
              Our Promise
            </div>
            ...
            <ul className="text-left max-w-md mx-auto space-y-3 mb-8">
              {[
                ...
                <li key={item} className="flex items-center gap-3 text-gray-300 text-sm">
                  <CheckCircle size={16} className="shrink-0" style={{ color: "#00adee" }} />
                  {item}
                </li>
              ))}
            </ul>
            <Link
              href="/booking"
              className="inline-flex items-center gap-2 font-bold text-white px-8 py-4 rounded-full transition-opacity hover:opacity-90"
